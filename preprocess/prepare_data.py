...
if __name__ == '__main__':
    ...
    print('output directory: {}'.format(opt.output_dir))

    # voc 2007
    print('')
    voc_path =os.path.join(opt.data_dir, 'train/VOCdevkit/VOC2007')
    voc_outpath = os.path.join(opt.output_dir, 'voc_train.txt')
    print('Read VOC dataset: {}'.format(voc_path))
    print('Write VOC txt: {}'.format(voc_outpath))
    data_prepare = VOCPrepare(voc_path, opt.class_name_dir, voc_outpath)
    data_prepare.write()

    print('')
    voc_path =os.path.join(opt.data_dir, 'train/VOCdevkit/VOC2012')
    voc_outpath = os.path.join(opt.output_dir, 'voc_train.txt')
    print('Read VOC dataset: {}'.format(voc_path))
    print('Write VOC txt: {}'.format(voc_outpath))
    data_prepare = VOCPrepare(voc_path, opt.class_name_dir, voc_outpath)
    ...
    print('')
    voc_path =os.path.join(opt.data_dir, 'test/VOCdevkit/VOC2007')
    voc_outpath = os.path.join(opt.output_dir, 'voc_test.txt')
    print('Read VOC dataset: {}'.format(voc_path))
    print('Write VOC txt: {}'.format(voc_outpath)) 
    data_prepare = VOCPrepare(voc_path, opt.class_name_dir, voc_outpath)
    data_prepare.write()
